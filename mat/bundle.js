...
var app = angular.module('heroApp', []).controller('MainCtrl', function MainCtrl($scope) {
    ...
    $scope.i2 = [431.76, Math.round(interpolate(0.11277, 0.115, 0.11678, 431.76, 435.99), 1), 440.21, 444.44, Math.round(interpolate(0.12951, 0.13, 0.13401, 448.67, 452.9), 1), Math.round(interpolate(0.13401, 0.135, 0.13863, 452.9, 457.9), 1), Math.round(interpolate(0.13863, 0.14, 0.14338, 457.9, 461.36), 1), Math.round(interpolate(0.14338, 0.145, 0.14826, 461.36, 465.60), 1), Math.round(interpolate(0.14826, 0.15, 0.15328, 465.60, 469.36), 1)];

    // Interpolated entalpia - heat

    $scope.i_n = [2680.3, Math.round(interpolate(0.11277, 0.115, 0.11678, 2680.3, 2681.8), 1), 2683.4, 2684.9, Math.round(interpolate(0.12951, 0.13, 0.13401, 2686.5, 2688.0), 1), Math.round(interpolate(0.13401, 0.135, 0.13863, 2688.0, 2689.5), 1), Math.round(interpolate(0.13863, 0.14, 0.14338, 2689.5, 2691.1), 1), Math.round(interpolate(0.14338, 0.145, 0.14826, 2691.1, 2692.6), 1), Math.round(interpolate(0.14826, 0.15, 0.15328, 2692.6, 2694.1), 1)];
    ...
    $scope.calcDSv = function (t_ll_sv, t_l_sv, i_ll, i_k, u_pod) {
        ...
        if (t_ll_sv && t_l_sv && i_ll && u_pod && i_k && $scope.g_sv) {
            ...
            for (var i = 0; i < i_k.length; i++) {
                var val = ($scope.g_sv * (t_ll_sv - t_l_sv) / (i_ll - i_k[i]) * u_pod).toFixed(3);
    ...
    $scope.calcTlllsv = function (t_ll_sv, i_ll_vip, i_k, u_pod) {
        if (t_ll_sv && i_ll_vip.length > 0 && i_k.length > 0 && u_pod && $scope.d_vip && $scope.g_hvo) {
            ...
            for (var i = 0; i < i_ll_vip.length; i++) {
                val = (t_ll_sv + $scope.d_vip * (i_ll_vip[i] - i_k[i]) / ($scope.g_hvo * u_pod)).toFixed(4);
    ...
    function relatePAndIntalpia(arr) {
        var p = [0.11, 0.115, 0.12, 0.125, 0.13, 0.135, 0.14, 0.145, 0.15];
        var str_arr = [];
  

        arr.forEach(function (element, i) {
            str_arr.push('\u041F\u0440\u0438 P = ' + p[i] + ', i = ' + element);
        });
        return str_arr;
    }
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: finalArr,
                    backgroundColor: ['rgba(255, 99, 132, 0.2)', 'rgba(54, 162, 235, 0.2)', 'rgba(255, 206, 86, 0.2)', 'rgba(75, 192, 192, 0.2)', 'rgba(153, 102, 255, 0.2)', 'rgba(255, 159, 64, 0.2)', 'rgba(89, 229, 220, 0.2)', 'rgba(152, 129, 226, 0.2)', 'rgba(249, 104, 208, 0.2)'],
                    borderColor: ['rgba(255,99,132,1)', 'rgba(54, 162, 235, 1)', 'rgba(255, 206, 86, 1)', 'rgba(75, 192, 192, 1)', 'rgba(153, 102, 255, 1)', 'rgba(255, 159, 64, 1)', 'rgb(89, 229, 220)', 'rgb(152, 129, 226)', 'rgb(249, 104, 208)'],
                    borderWidth: 1
                }, {
                    label: '9% от D',
                    data: lineArr,
                    backgroundColor: 'rgb(104, 249, 135)',

                    borderColor: 'rgb(104, 249, 135)',
